...
<div class="max-w-7xl mx-auto py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="<?php echo e(route('orders.index')); ?>" class="text-blue-500 hover:underline flex items-center">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-900"><?php echo e($order->created_at->format('M d, Y, h:i A')); ?></p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500">Total Amount</h3>
                    <p class="mt-1 text-sm text-gray-900">$<?php echo e(number_format($order->total_price, 2)); ?></p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500">Order Status</h3>
                    <p class="mt-1 text-sm text-gray-900"><?php echo e(ucfirst($order->status)); ?></p>
                </div>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php $__currentLoopData = $order->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?php echo e($item->product_title); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">$<?php echo e(number_format($item->product_price, 2)); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right"><?php echo e($item->quantity); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-right">$<?php echo e(number_format($item->product_price * $item->quantity, 2)); ?></td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <!-- Order Total -->
                    <tr class="bg-gray-50">
                        <td colspan="3" class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-right">Total:</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">$<?php echo e(number_format($order->total_price, 2)); ?></td>
                    </tr>
                </tbody>
            </table>
        </div>
                                ...
                                <div class="relative pb-8">
                                    ...
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                <p class="text-sm text-gray-500">Order status changed to <span class="font-medium text-gray-900"><?php echo e(ucfirst($history->status)); ?></span></p>
                                                <?php if($history->comment): ?>
                                                    <p class="mt-1 text-sm text-gray-500"><?php echo e($history->comment); ?></p>
                                                <?php endif; ?>
                                            </div>
                                            <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                                <time datetime="<?php echo e($history->created_at); ?>"><?php echo e($history->created_at->format('M d, Y, h:i A')); ?></time>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\BBvb-main\resources\views/orders/show.blade.php ENDPATH**/ ?>