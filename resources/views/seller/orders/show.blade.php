...
<div class="max-w-7xl mx-auto py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('seller.orders.index') }}" class="text-blue-500 hover:underline flex items-center">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($order->items as $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500">${{ number_format($item->product_price, 2) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500">{{ $item->quantity }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">${{ number_format($item->product_price * $item->quantity, 2) }}</td>
                                </tr>
                            @endforeach
                            <tr class="bg-gray-50">
                                <td colspan="3" class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">Order Total:</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold">${{ number_format($order->total_price, 2) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                                    ...
                                    <div class="relative pb-8">
                                        ...
                                        <div class="relative flex space-x-3">
                                            ...
                                            <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                                <div>
                                                    <p class="text-sm text-gray-500">Status changed to <span class="font-medium text-gray-900">{{ ucfirst($history->status) }}</span></p>
                                                    @if($history->comment)
                                                        <p class="mt-1 text-sm text-gray-500">{{ $history->comment }}</p>
                                                    @endif
                                                </div>
                                                <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                                    <time datetime="{{ $history->created_at->format('Y-m-d') }}">{{ $history->created_at->format('M d, Y, h:i A') }}</time><br>
                                                    <span>by {{ $history->creator ? $history->creator->name : 'System' }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
